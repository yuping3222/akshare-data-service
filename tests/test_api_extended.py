...
def make_df(columns=None, rows=5):
    cols = columns or {"date": pd.date_range("2024-01-01", periods=rows), "value": [1.0] * rows}
...
class TestMacroChinaAPI:
    ...
    def test_interest_rate(self):
        service = DataService()
        test_df = make_df({"date": pd.date_range("2024-01-01", periods=5), "rate": [2.0] * 5})
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_shibor_rate", return_value=test_df):
    ...
    def test_gdp(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_macro_gdp", return_value=test_df):
                    df = service.macro.china.gdp("2024-01-01", "2024-12-31")
                    assert not df.empty

    def test_social_financing(self):
        service = DataService()
        test_df = make_df({"date": pd.date_range("2024-01-01", periods=5), "total": [3000000.0] * 5})
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_social_financing", return_value=test_df):
                    df = service.macro.china.social_financing("2024-01-01", "2024-01-10")
                    assert not df.empty

    def test_interest_rate_cached(self):
        service = DataService()
        cached = make_df({"date": pd.date_range("2024-01-01", periods=5), "rate": [2.0] * 5})
...
class TestCNStockCapitalAPI:
    ...
    def test_northbound_holdings(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_northbound_holdings", return_value=test_df):
                    df = service.cn.stock.capital.northbound_holdings("600000", "2024-01-01", "2024-01-10")
                    assert not df.empty
    ...
    def test_block_deal(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_block_deal", return_value=test_df):
    ...
    def test_block_deal_with_symbol(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_block_deal", return_value=test_df):
                    df = service.cn.stock.capital.block_deal("600000", "2024-01-01", "2024-01-10")
                    assert not df.empty
    ...
    def test_dragon_tiger(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_dragon_tiger_list", return_value=test_df):
    ...
    def test_margin(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_margin_data", return_value=test_df):
    ...
    def test_north(self):
        service = DataService()
        test_df = make_df({"date": pd.date_range("2024-01-01", periods=5), "north_money": [1000.0] * 5})
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_north_money_flow", return_value=test_df):
...
class TestCNStockEventAPI:
    ...
    def test_dividend(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_dividend_data", return_value=test_df):
    ...
    def test_restricted_release(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_restricted_release", return_value=test_df):
...
class TestCNStockFinanceAPI:
    ...
    def test_income_statement(self):
        ...
        with patch.object(service.akshare, "get_income_statement", return_value=test_df):
...
class TestCNIndexMetaAPI:
    ...
    def test_components(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_index_components", return_value=test_df):
...
class TestExecuteSourceMethodErrorPaths:
    ...
    def test_execute_source_method_no_matching_source(self):
        ...
        result = service._execute_source_method(
            "unknown_method", "nonexistent_source"
        )
    ...
    def test_execute_source_method_with_router_fallback(self):
        ...
        with patch.object(service.akshare, "get_daily_data", return_value=make_df()):
            result = service._execute_source_method("get_daily_data", "akshare", "600000", "2024-01-01", "2024-01-10", "qfq")
...
class TestDataServiceFinancialMethods:
    ...
    def test_get_financial_metrics(self):
        ...
        with patch.object(service.akshare, "get_financial_metrics", return_value=test_df):
...
class TestDataServiceDataFacade:
    ...
    def test_get_northbound_holdings(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_northbound_holdings", return_value=test_df):
                    df = service.get_northbound_holdings("600000", "2024-01-01", "2024-01-10")
                    assert not df.empty
...
class TestDataServiceConceptBoard:
    ...
    def test_get_concept_stocks(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_concept_components", return_value=test_df):
...
class TestDataServiceExtendedMethods:
    ...
    def test_get_fund_open_nav(self):
        service = DataService()
        test_df = make_df({"date": pd.date_range("2024-01-01", periods=5), "nav": [1.5] * 5})
        mock_proxy = MagicMock()
    ...
    def test_get_sw_industry_daily(self):
        service = DataService()
        test_df = make_df({"date": pd.date_range("2024-01-01", periods=5), "close": [3000.0] * 5})
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                mock_proxy = MagicMock()
                mock_proxy.get_sw_index_daily.return_value = test_df
                with patch.object(service, "_get_source", return_value=mock_proxy):
                    df = service.get_sw_industry_daily("801010", "2024-01-01", "2024-01-10")
                    assert not df.empty
...
class TestDataServiceEquityPledge:
    ...
    def test_get_equity_pledge(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_equity_pledge", return_value=test_df):
    ...
    def test_get_equity_pledge_rank(self):
        ...
        with patch.object(service.akshare, "get_equity_pledge_rank", return_value=test_df):
...
class TestDataServiceGoodwill:
    ...
    def test_get_goodwill_data(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_goodwill_data", return_value=test_df):
    ...
    def test_get_goodwill_impairment(self):
        ...
        with patch.object(service.akshare, "get_goodwill_impairment", return_value=test_df):
    ...
    def test_get_goodwill_by_industry(self):
        ...
        with patch.object(service.akshare, "get_goodwill_by_industry", return_value=test_df):
...
class TestDataServiceRepurchase:
    ...
    def test_get_repurchase_data(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_repurchase_data", return_value=test_df):
                    df = service.get_repurchase_data("600000", "2024-01-01", "2024-01-10")
                    assert not df.empty
...
class TestDataServiceESG:
    ...
    def test_get_esg_rating(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_esg_rating", return_value=test_df):
...
class TestDataServicePerformance:
    ...
    def test_get_performance_forecast(self):
        service = DataService()
        test_df = make_df({"symbol": ["600000"], "forecast_eps": [1.0]})
        with patch.object(service.akshare, "get_performance_forecast", return_value=test_df):
    ...
    def test_get_performance_express(self):
        ...
        with patch.object(service.akshare, "get_performance_express", return_value=test_df):
...
class TestDataServiceChipDistribution:
    ...
    def test_get_chip_distribution(self):
        ...
        with patch.object(service.akshare, "get_chip_distribution", return_value=test_df):
...
class TestDataServiceBonus:
    ...
    def test_get_stock_bonus(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_stock_bonus", return_value=test_df):
    ...
    def test_get_rights_issue(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_rights_issue", return_value=test_df):
    ...
    def test_get_dividend_by_date(self):
        ...
        with patch.object(service.akshare, "get_dividend_by_date", return_value=test_df):
...
class TestDataServiceCompanyInfo:
    ...
    def test_get_management_info(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_management_info", return_value=test_df):
    ...
    def test_get_name_history(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_name_history", return_value=test_df):
...
class TestDataServiceMacroExtended:
    ...
    def test_get_social_financing(self):
        service = DataService()
        test_df = make_df({"date": pd.date_range("2024-01-01", periods=5), "total": [3e6] * 5})
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_social_financing", return_value=test_df):
    ...
    def test_get_macro_exchange_rate(self):
        service = DataService()
        test_df = make_df({"date": pd.date_range("2024-01-01", periods=5), "usd_cny": [7.2] * 5})
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_macro_exchange_rate", return_value=test_df):
...
class TestDataServiceFundMethods:
    ...
    def test_get_fof_list(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_fof_list", return_value=test_df):
                    df = service.get_fof_list()
                    assert not df.empty

    def test_get_fof_nav(self):
        service = DataService()
        test_df = make_df({"date": pd.date_range("2024-01-01", periods=5), "nav": [1.2] * 5})
    ...
    def test_get_lof_spot(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_lof_spot", return_value=test_df):
...
class TestDataServiceConvertBond:
    ...
    def test_get_convert_bond_premium(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_convert_bond_premium", return_value=test_df):
    ...
    def test_get_convert_bond_spot(self):
        ...
        with patch.object(service.akshare, "get_convert_bond_spot", return_value=test_df):
    ...
    def test_get_conversion_bond_list(self):
        ...
        with patch.object(service.akshare, "get_conversion_bond_list", return_value=test_df):
            df = service.get_conversion_bond_list()
            assert not df.empty

    def test_get_conversion_bond_daily(self):
        service = DataService()
        test_df = make_df({"date": pd.date_range("2024-01-01", periods=5), "close": [120.0] * 5})
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_conversion_bond_daily", return_value=test_df):
                    df = service.get_conversion_bond_daily("113009", "2024-01-01", "2024-01-10")
                    assert not df.empty
...
class TestDataServiceIndustry:
    ...
    def test_get_industry_performance(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_industry_performance", return_value=test_df):
    ...
    def test_get_concept_performance(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_concept_performance", return_value=test_df):
...
class TestDataServiceHotRank:
    ...
    def test_get_hot_rank(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_hot_rank", return_value=test_df):
...
class TestDataServiceOption:
    ...
    def test_get_option_daily(self):
        service = DataService()
        test_df = make_df({"date": pd.date_range("2024-01-01", periods=5), "close": [0.15] * 5})
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_option_daily", return_value=test_df):
...
class TestDataServiceLOF:
    ...
    def test_get_lof_daily(self):
        service = DataService()
        test_df = make_df({"date": pd.date_range("2024-01-01", periods=5), "close": [1.5] * 5})
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_lof_hist", return_value=test_df):
...
class TestDataServiceFutures:
    ...
    def test_get_futures_daily(self):
        service = DataService()
        test_df = make_df({"date": pd.date_range("2024-01-01", periods=5), "close": [5000.0] * 5})
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_futures_daily", return_value=test_df):
...
class TestDataServiceStockHist:
    ...
    def test_get_stock_hist_daily(self):
        service = DataService()
        test_df = make_df({"date": pd.date_range("2024-01-01", periods=5), "close": [10.5] * 5})
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_daily_data", return_value=test_df):
                    df = service.get_stock_hist("600000", "daily", "2024-01-01", "2024-01-10")
                    assert not df.empty

    def test_get_stock_hist_minute(self):
        service = DataService()
        test_df = make_df({"datetime": pd.date_range("2024-01-01", periods=5, freq="min"), "close": [10.5] * 5})
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_minute_data", return_value=test_df):
                    df = service.get_stock_hist("600000", "1min", "2024-01-01", "2024-01-01")
                    assert not df.empty
...
class TestDataServiceRestrictedReleaseCalendar:
    ...
    def test_get_restricted_release_calendar(self):
        ...
        with patch.object(service.akshare, "get_restricted_release_calendar", return_value=test_df):
...
class TestDataServiceIndexComponents:
    ...
    def test_get_index_components(self):
        service = DataService()
        test_df = make_df({"index_code": ["000300"], "code": ["600000"], "weight": [0.5]})
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_index_components", return_value=test_df):
    ...
    def test_get_index_components_cached(self):
        service = DataService()
        cached = make_df({"index_code": ["000300"], "code": ["600000"], "weight": [0.5]})
        with patch.object(service.cache, "read", return_value=cached):
            df = service.get_index_components("000300")
            assert not df.empty
...
class TestDataServiceSecuritiesList:
    ...
    def test_get_securities_list(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_securities_list", return_value=test_df):
...
class TestDataServiceSuspendedST:
    ...
    def test_get_suspended_stocks(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_suspended_stocks", return_value=test_df):
    ...
    def test_get_st_stocks(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_st_stocks", return_value=test_df):
...
class TestDataServiceIndustryStocks:
    ...
    def test_get_industry_stocks(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.lixinger, "get_industry_stocks", return_value=["600000", "600519"]):
                    with patch.object(service.akshare, "get_industry_stocks", return_value=["600000", "600519"]):
                        stocks = service.get_industry_stocks("801010")
                        assert len(stocks) == 2

    def test_get_industry_stocks_cached(self):
        service = DataService()
        cached = pd.DataFrame({"industry_code": ["801010", "801010"], "level": [1, 1], "code": ["600000", "600519"]})
        with patch.object(service.cache, "read", return_value=cached):
            stocks = service.get_industry_stocks("801010")
            assert len(stocks) == 2
...
class TestDataServiceCallAuction:
    ...
    def test_get_call_auction(self):
        ...
        with patch.object(service.cache, "read", return_value=pd.DataFrame()):
            with patch.object(service.cache, "write", return_value=""):
                with patch.object(service.akshare, "get_call_auction", return_value=test_df):

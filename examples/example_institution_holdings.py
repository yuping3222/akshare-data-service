"""
机构持股接口示例 (get_institution_holdings)

演示如何获取股票的机构持股数据，包括基金、券商、保险等
机构投资者的持股情况。

接口说明:
- get_institution_holdings(symbol): 获取指定股票的机构持股信息

参数:
  symbol: 股票代码，支持多种格式 (如 "600519", "000001", "sh600519")

返回: DataFrame，包含机构名称、持股数量、持股比例、机构类型等字段
...
def example_basic():
    """基本用法: 获取贵州茅台的机构持股"""
    print("=" * 60)
    print("示例 1: 基本用法 - 获取贵州茅台(600519)机构持股")
    print("=" * 60)
    ...
    try:
        # symbol: 证券代码
        df = service.get_institution_holdings(symbol="600519")
...
def example_with_source():
    ...
    try:
        ...
        df = service.get_institution_holdings(
            symbol="600519",
            source="lixinger",
        )
...
def example_sz_stock():
    """获取深市股票的机构持股"""
    print("\n" + "=" * 60)
    print("示例 3: 获取平安银行(000001)机构持股")
    print("=" * 60)

    service = get_service()

    try:
        df = service.get_institution_holdings(symbol="000001")
...
def example_multiple_stocks():
    ...
    for symbol in symbols:
        try:
            df = service.get_institution_holdings(symbol=symbol)
            if df is None or df.empty:
                print(f"\n{symbol}: 无数据")
            else:
                print(f"\n{symbol}: 共 {len(df)} 条记录")

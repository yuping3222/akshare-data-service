...
def example_basic():
    ...
    try:
        ...
        df = _fetch_with_retry(
            lambda: service.get_capital_change(symbol="大笔买入"),
            "get_capital_change(大笔买入)",
        )
...
def example_sz_stock():
    ...
    for change_type in change_types:
        try:
            df = _fetch_with_retry(
                lambda t=change_type: service.get_capital_change(symbol=t),
                f"get_capital_change({change_type})",
            )
...
def example_analysis():
    ...
    try:
        df = _fetch_with_retry(
            lambda: service.get_capital_change(symbol="火箭发射"),
            "get_capital_change(analysis)",
        )
...
def example_error_handling():
    ...
    print("\n测试 2: 无效异动类型")
    try:
        # 也演示 symbol 规范化对无效代码输入的降级处理
        invalid = _normalize_symbol("INVALID_TYPE")
        df = _fetch_with_retry(
            lambda: service.get_capital_change(symbol=invalid),
            "get_capital_change(INVALID_TYPE)",
        )
        if df is None:
            print("  结果: 无数据")
        else:

"""get_shareholder_changes 示例（symbol 规范 + 重试 + 降级）。"""
...
def main() -> None:
    ...
    for raw_symbol in ["600519", "sh600519", "000001.XSHE"]:
        ...
        df = _fetch_with_retry(
            lambda s=symbol: service.get_shareholder_changes(symbol=s),
            f"get_shareholder_changes({raw_symbol})",
        )
        if df is None:

...
class TestFetchDailyData:
    ...
    def test_stock_daily(self):
        ...
        with patch.object(fetcher, "fetch", return_value=mock_df) as mock_fetch:
            df = fetch_daily_data(ak, symbol="000001", start_date="20240101", end_date="20240201")
            assert isinstance(df, pd.DataFrame)
            assert not df.empty
            mock_fetch.assert_called_once_with(
                "equity_daily", akshare=ak, symbol="000001",
                start_date="20240101", end_date="20240201", adjust="qfq",
            )
...
class TestFetchMacroAliases:
    ...
    def test_macro_gdp(self):
        ...
        with patch.object(fetcher, "fetch", return_value=mock_df) as mock_fetch:
            ...
            assert isinstance(df, pd.DataFrame)
            assert not df.empty
            mock_fetch.assert_called_once_with("macro_gdp", akshare=ak, start_date=None, end_date=None)

    @pytest.mark.integration
    def test_macro_exchange_rate(self):
        mock_df = pd.DataFrame({
            "date": pd.date_range("2024-01-01", periods=10, freq="D"),
            "exchange_rate": [7.1, 7.12, 7.08, 7.15, 7.11, 7.09, 7.13, 7.10, 7.14, 7.07],
        })
        with patch.object(fetcher, "fetch", return_value=mock_df) as mock_fetch:
            df = fetch_macro_exchange_rate(ak)
            assert isinstance(df, pd.DataFrame)
            mock_fetch.assert_called_once_with(
                "macro_exchange_rate", akshare=ak, start_date=None, end_date=None,
            )
...
class TestFetchFinancialAliases:
    ...
    def test_finance_indicator(self):
        ...
        with patch.object(fetcher, "fetch", return_value=mock_df) as mock_fetch:
            ...
            assert isinstance(df, pd.DataFrame)
            mock_fetch.assert_called_once_with(
                "finance_indicator", akshare=ak, symbol="000001",
    ...
    def test_balance_sheet(self):
        ...
        with patch.object(fetcher, "fetch", return_value=mock_df) as mock_fetch:
            ...
            assert isinstance(df, pd.DataFrame)
            mock_fetch.assert_called_once_with(
                "balance_sheet", akshare=ak, symbol="000001",
                start_date=None, end_date=None,
            )
    ...
    def test_income_statement(self):
        ...
        with patch.object(fetcher, "fetch", return_value=mock_df) as mock_fetch:
            ...
            assert isinstance(df, pd.DataFrame)
            mock_fetch.assert_called_once_with(
                "income_statement", akshare=ak, symbol="000001",
                start_date=None, end_date=None,
            )
    ...
    def test_cash_flow(self):
        ...
        with patch.object(fetcher, "fetch", return_value=mock_df) as mock_fetch:
            ...
            assert isinstance(df, pd.DataFrame)
            mock_fetch.assert_called_once_with(
                "cash_flow", akshare=ak, symbol="000001",
                start_date=None, end_date=None,
            )
...
class TestFetchMarketAliases:
    ...
    def test_money_flow(self):
        ...
        with patch.object(fetcher, "fetch", return_value=mock_df) as mock_fetch:
            ...
            assert isinstance(df, pd.DataFrame)
            mock_fetch.assert_called_once_with(
                "money_flow", akshare=ak, symbol="000001", start_date=None, end_date=None,
            )
    ...
    def test_north_money_flow(self):
        ...
        with patch.object(fetcher, "fetch", return_value=mock_df) as mock_fetch:
            ...
            assert isinstance(df, pd.DataFrame)
            mock_fetch.assert_called_once_with(
                "north_money_flow", akshare=ak, start_date=None, end_date=None,
            )
    ...
    def test_block_deal(self):
        ...
        with patch.object(fetcher, "fetch", return_value=mock_df) as mock_fetch:
            ...
            assert isinstance(df, pd.DataFrame)
            mock_fetch.assert_called_once_with("block_deal", akshare=ak, date="2024-01-15")
    ...
    def test_margin_summary(self):
        ...
        with patch.object(fetcher, "fetch", return_value=mock_df) as mock_fetch:
            df = fetch_margin_summary(ak, start_date="2024-01-01", end_date="2024-01-05")
            assert isinstance(df, pd.DataFrame)
            mock_fetch.assert_called_once_with(
                "margin_summary", akshare=ak, start_date="2024-01-01", end_date="2024-01-05",
            )
...
class TestFetchListAliases:
    ...
    def test_index_stocks(self):
        ...
        with patch.object(fetcher, "fetch", return_value=mock_df) as mock_fetch:
            ...
            assert isinstance(df, pd.DataFrame)
            mock_fetch.assert_called_once_with(
                "index_components", akshare=ak, symbol="000300",
            )
...
class TestFetchFundAliases:
    ...
    def test_fund_net_value(self):
        ...
        with patch.object(fetcher, "fetch", return_value=mock_df) as mock_fetch:
            ...
            assert isinstance(df, pd.DataFrame)
            mock_fetch.assert_called_once_with(
                "fund_net_value", akshare=ak, symbol="510050",
                start_date=None, end_date=None,
            )
    ...
    def test_sector_fund_flow(self):
        ...
        with patch.object(fetcher, "fetch", return_value=mock_df) as mock_fetch:
            ...
            assert isinstance(df, pd.DataFrame)
            mock_fetch.assert_called_once_with(
                "sector_fund_flow", akshare=ak, date=None, sector_type="industry",
            )
...
class TestFetchIndustryAliases:
    ...
    def test_industry_stocks(self):
        ...
        with patch.object(fetcher, "fetch", return_value=mock_df) as mock_fetch:
            ...
            mock_fetch.assert_called_once_with(
                "industry_stocks", akshare=ak, symbol="801010",
            )
    ...
    def test_industry_mapping(self):
        ...
        with patch.object(fetcher, "fetch", return_value=mock_df) as mock_fetch:
            ...
            assert isinstance(df, pd.DataFrame)
            mock_fetch.assert_called_once_with(
                "industry_mapping", akshare=ak, symbol="801010",
            )

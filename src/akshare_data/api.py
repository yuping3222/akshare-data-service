"""Online data service: Read-only Served facade.

DataService is now a read-only facade that serves data from the Served layer.
It NEVER synchronously fetches from source adapters.

Backward compatibility:
- All existing method signatures are preserved
- Imports like `from akshare_data import DataService` still work
- Namespace API (service.cn.stock.quote.daily, etc.) still works
- When Served has no data, returns empty DataFrame (not silent source fetch)
"""

...
class DataSource(ABC):
    ...
    def get_financial_analysis_indicator(self, symbol: str) -> pd.DataFrame:
        """获取财务分析指标数据"""
        raise NotImplementedError(f"{self.name} 不支持 get_financial_analysis_indicator")
    ...
    def get_index_stock_cons_weight_csindex(self, symbol: str) -> pd.DataFrame:
        """获取中证指数成分股及权重"""
        raise NotImplementedError(f"{self.name} 不支持 get_index_stock_cons_weight_csindex")
    ...
    def get_fund_announcement_dividend_em(self) -> pd.DataFrame:
        """获取基金分红公告"""
        raise NotImplementedError(f"{self.name} 不支持 get_fund_announcement_dividend_em")
    ...
    def get_futures_settlement_price_sina(self, symbol: str) -> pd.DataFrame:
        """获取期货结算价（新浪）"""
        raise NotImplementedError(f"{self.name} 不支持 get_futures_settlement_price_sina")

    def get_futures_contract_info_shfe_dce_czce(self) -> pd.DataFrame:
        """获取期货合约信息"""
        raise NotImplementedError(f"{self.name} 不支持 get_futures_contract_info_shfe_dce_czce")
    ...
    def get_stock_hsgt_fund_flow_summary_em(self) -> pd.DataFrame:
        """获取沪深港通资金流向汇总"""
        raise NotImplementedError(f"{self.name} 不支持 get_stock_hsgt_fund_flow_summary_em")
    ...
    def get_stock_em_performance_letters(self, symbol: str) -> pd.DataFrame:
        """获取业绩快报（东方财富）"""
        raise NotImplementedError(f"{self.name} 不支持 get_stock_em_performance_letters")
